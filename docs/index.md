...
# run app
java -jar target/java-harness-two-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-harness-two:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-harness-two -n dev

# get external endpoint
kubectl get svc java-harness-two-svc -n dev
```
...
```bash
kubectl port-forward svc/java-harness-two-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-harness-two -n dev
kubectl rollout status deploy/java-harness-two -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javaharnesstwo/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javaharnesstwo/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
